// src/screens/HomeScreen.js - ACTUALIZADO CON SINCRONIZACIÓN DE NOTIFICACIONES
...
const HomeScreen = ({ navigation }) => {
  ...
  useEffect(() => {
  if (careers.length > 0 || tours.length > 0) {

    // 🟢 SOLO crear notificaciones si NO es la primera carga
    if (prevCareersRef.current.length > 0 || prevToursRef.current.length > 0) {
      checkForUpdates(
        careers,
        tours,
      ...
      console.log("📦 Primera carga: NO se generarán notificaciones");
    }

    // 🟢 Actualizar referencias DESPUÉS
  ...
  // ✅ Handler para iniciar un tour AR O VR 360° desde "Tours Disponibles"
  const handleTourPress = (tour) => {
    console.log('🎬 Tour seleccionado desde Tours Disponibles:', tour.title);
    console.log('📋 Datos del tour:', { type: tour.type, multimedia: tour.multimedia });
    
    // ✅ DETECTAR TIPO: Primero por campo "type", luego por multimedia
    let tourType = 'AR'; // Default
    ...
    console.log('🔍 Tipo detectado:', tourType);

    if (tourType === '360') {
      // 🥽 Navegar a VR360Viewer
      console.log('🥽 Abriendo tour VR 360°:', tour.title);
      navigation.navigate('VR360Viewer', {
        tourId: tour.id || tour._id,
      });
    } else {
      // 📱 Navegar a ARViewer
      console.log('📱 Abriendo tour AR:', tour.title);
  ...
  // ✅ Handler para abrir tours desde "Tours Recientes" usando tourType guardado
  const handleRecentTourPress = (watchedTour) => {
    ...
    // ✅ USAR EL TIPO GUARDADO EN EL HISTORIAL (más confiable)
    const tourType = watchedTour.tourType || 'AR';

    console.log('🔍 Tour desde recientes:', {
      title: fullTour.title,
      tourType: tourType,
      watchedAt: watchedTour.watchedAt
    });

    if (tourType === '360') {
      // 🥽 Tour VR 360°
      console.log('🥽 Abriendo tour VR 360° desde recientes:', fullTour.title);
      navigation.navigate('VR360Viewer', {
        tourId: fullTour.id || fullTour._id,
      });
    } else {
      // 📱 Tour AR
      console.log('📱 Abriendo tour AR desde recientes:', fullTour.title);
      navigation.navigate('ARViewer', {
        tourId: fullTour.id || fullTour._id,
      ...
      {isLoading ? (
        ...
        >
          ...
          <View style={styles.section}>
            ...
            {(() => {
              // ✅ ORDENAR TOURS POR FECHA DE CREACIÓN (MÁS RECIENTES PRIMERO)
              ...
              // Tomar solo los primeros 10 tours más recientes
              const newTours = sortedTours.slice(0, 10);

              // Verificar si hay tours realmente nuevos (menos de 7 días)
              const now = Date.now();
              ...
              return newTours.length === 0 ? (
                ...
                newTours.map((tour) => {
                  ...
                  // ✅ VERIFICAR SI ES REALMENTE NUEVO (menos de 7 días)
                    ...
                    >
                      <TouchableOpacity 
                        style={[
                          ...
                          isReallyNew && styles.newTourCard, // ✅ Estilo especial para tours nuevos
                        ]}
                      ...
                      >
                        <View style={styles.tourImageContainer}>
                          ...
                          {/* ✅ BADGE "NUEVO" para tours recientes */}
                          {isReallyNew && (
                        ...
                        <View style={styles.tourInfo}>
                          ...
                          {/* ✅ MOSTRAR FECHA DE AGREGADO para tours nuevos */}
...
const styles = StyleSheet.create({
  ...
  // ✅ ESTILOS NUEVOS PARA "NUEVOS TOURS"
newTourCard: {
  borderLeftWidth: 4,
  borderLeftColor: '#10B981',
